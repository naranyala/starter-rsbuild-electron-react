...
(() => {
 ...
 self.max &&*/
  ...
  function U(a, b) {
    ...
    this.g.className =
      'winbox' + (Z ? ' ' + ('string' === typeof Z ? Z : Z.join(' ')) : '') + (fa ? ' modal' : '');
    this.g.winbox = this;
    this.window = this.g;
    ...
    ea
      ...
      : ((q = q ? V(q, y) : Math.max(y / 2, w) | 0), (r = r ? V(r, z) : Math.max(z / 2, C) | 0));
    X = X ? V(X, c, q) : N;
    Y = Y ? V(Y, v, r) : M;
    this.x = X;
    this.y = Y;
  ...
  function ja() {
    ...
    ha =
      J &&
      J.replace('request', 'exit').replace('mozRequest', 'mozCancel').replace('Request', 'Exit');
    k(window, 'resize', () => {
      na();
      oa();
    });
  ...
  function ka(a) {
    ...
    W(a, 'se');
    W(a, 'sw');
    k(a.g.getElementsByClassName('wb-min')[0], 'click', (b) => {
      t(b);
      a.min ? a.restore().focus() : a.minimize();
    });
    k(a.g.getElementsByClassName('wb-max')[0], 'click', (b) => {
      t(b);
      a.max ? a.restore().focus() : a.maximize().focus();
    });
    J
      ? k(a.g.getElementsByClassName('wb-full')[0], 'click', (b) => {
          t(b);
          a.fullscreen().focus();
        })
      : a.addClass('no-full');
    k(a.g.getElementsByClassName('wb-close')[0], 'click', (b) => {
      t(b);
      a.close() || (a = null);
    });
    k(
  ...
  function W(a, b) {
    ...
    function f(g) {
      ...
      I &&
        ((a.height = Math.max(Math.min(a.height, a.l, P - a.y - a.bottom), a.o)),
        (I = a.height !== y));
      (H || I) && a.resize();
      G &&
        (a.max &&
          (a.x =
            (q < K / 3 ? a.left : q > (K / 3) * 2 ? K - a.width - a.right : K / 2 - a.width / 2) +
            r),
        (a.x = Math.max(
